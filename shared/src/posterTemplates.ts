...
const ROLE_META: {
...
}[] = [
  {
    ...
    roleLabel: "溯源",
    layoutHint:
      "横版手抄报（宽屏16:9）：顶部左侧大标题+圆角提问小标题，右侧可放小组名；画面用一条从左到右的横向时间带/道路从古到今串联；沿途至少3个彩色圆角内容框左右铺开，每框配2～3个阶段小插画；底部左侧「为什么变好了」图标条（4个小圆标）+感悟框，右下角背书包小学生和对话框；禁止做成科技发明卡片矩阵或古/今/未来三栏畅想；强制横构图宽大于高",
    visualFingerprint:
  ...
  },
  {
    ...
    roleLabel: "赋能",
    layoutHint:
      "横版手抄报（宽屏16:9）：顶部左侧大标题+圆角提问小标题；中间横向排列 4～6 张圆角科技发明卡片（每张含发明插画+短说明），卡片之间用虚线/箭头连接；下方一条「科技带来什么」四图标横条；右下/底部探索精神感悟框+小学生角色对话框；禁止做成完整演变时间线或古/今/未来三栏；强制横构图宽大于高",
    visualFingerprint:
  ...
  },
  {
    ...
    roleLabel: "畅想",
    layoutHint:
      "横版手抄报（宽屏16:9）：顶部大标题+圆角提问小标题；横向三栏「古」痛点、「今」改善、「未来」畅想（三栏圆角框，各配插画）；底部感悟大框+小学生角色说「我要学习这种精神」类短句；点缀小植物/云朵装饰；禁止做成多段时间线或发明卡片矩阵；强制横构图宽大于高",
    visualFingerprint:
...
export function buildPosterPrompt(
  ...
  const techHint =
    template.role === "tech"
      ...
      : "";
  return [
    "请生成一张横版（宽屏16:9）小学生课堂手抄报海报，参考「科技改变出行」类精品手抄报：元素丰富、分区多、但不杂乱。",
    ...
    "画面要像可投屏的完整课堂手抄报：层次分明、装饰充足、适合小学三年级；强制横构图，宽明显大于高，适合教室投影横屏。",
  ].join("\n");
}
